...
@brd_routes.route('/brd/get-brd', methods=['POST'])
def get_workspace_brd_payload():
    """Get complete BRD for a workspace using payload"""
    try:
...
@brd_routes.route('/brd/get-summary', methods=['POST'])
def get_brd_summary_payload():
    """Get BRD summary with counts using payload"""
    try:
        data = request.get_json()
        if not data or not data.get('workspace_id'):
            return jsonify({'error': 'workspace_id is required'}), 400
...
@brd_routes.route('/brd/get-section', methods=['POST'])
def get_brd_section_payload():
    """Get specific BRD section using payload"""
    try:
        data = request.get_json()
